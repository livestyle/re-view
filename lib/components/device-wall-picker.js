...
import {deviceWallFSM as fsm} from '../app';
...
export default tr.component({
    render(props={}) {
        ...
        return <div className={cl('picker')}>
            <section className={cl('picker-section')}>
                <h2 className={cl('picker-section-title')}>Devices <i className={cl('icon-add', deviceEditMode && 'icon-add_active')}  onclick={addDevice}></i></h2>
            ...
            <section className={cl('picker-section')}>
                <h2 className={cl('picker-section-title')}>Presets <i className={cl('icon-add', presetEditMode && 'icon-add_active')} onclick={addPreset}></i></h2>
                <div className={cl('picker-section-content')}>
...
function getDeviceList(state) {
    ...
    var mode = 'pick-one';
    var items = getItems(state, 'devices').map(item => ({
        ...item,
...
function getPresetList(state) {
    ...
    return {
        items: markSelected(getItems(state, 'presets'), selected)
    };
}
