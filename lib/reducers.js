'use strict';

import {combineReducers} from 'redux';
import deepmerge from 'deepmerge';
import {DEVICE_WALL, USER, UI, OPTIONS, APP, DONATION, BREAKPOINTS} from './action-names';
import {getItems} from './helpers';

const keyReducers = combineReducers({
    devices: noop,
    presets: noop,
    pageUrl: noop,
    breakpoints,
    donation,
    options,
    deviceWallPicker,
    ui,
    user
});

export default function(state, action) {
    state = keyReducers(state, action);
    state = mergeState(state, action);
    return validateDeviceWallSelectedItem(state);
}

function noop(state={}) {
    return state;
}

function deviceWallPicker(state={}, action) {
    switch (action.type) {
        case DEVICE_WALL.SET_SELECTED:
            if (!state.display || state.display.id !== action.item.id) {
                state = {...state, display: action.item};
            }
            return state;

        case DEVICE_WALL.SET_STATE:
            return {
                ...state,
                state: action.state,
                stateData: action.data
            };

        case DEVICE_WALL.TOGGLE_VISIBILITY:
            return {
                ...state,
                visible: !state.visible
            };

        case UI.SET_MODE:
            if (state.visible) {
                return {
                    ...state,
                    visible: !state.visible
                };
            }
            break;
    }

    return state;
}

function user(state={}, action) {
    switch (action.type) {
        case USER.SAVE_DEVICE:
            let devices = removeItem(state.devices || [], action.device.id);
            devices.push(action.device);
            return {...state, devices};

        case USER.REMOVE_DEVICE:
            return {
                ...state,
                devices: removeItem(state.devices || [], action.id)
            };

        case USER.SAVE_PRESET:
            let presets = removeItem(state.presets || [], action.preset.id);
            presets.push(action.preset);
            return {...state, presets};

        case USER.REMOVE_PRESET:
            return {
                ...state,
                presets: removeItem(state.presets || [], action.id)
            };
    }

    return state;
}

function ui(state={}, action) {
    switch (action.type) {
        case UI.SET_MODE:
            if (action.mode && action.mode !== state.mode) {
                state = {
                    ...state,
                    mode: action.mode,
                    popup: null
                };
            }
            return state;
        case UI.SET_POPUP:
            if (action.popup !== state.popup) {
                state = {...state, popup: action.poup};
            }
            return state;
        case UI.TOGGLE_POPUP:
            return {
                ...state,
                popup: state.popup === action.popup ? null : action.popup
            };
        case UI.RESET_POPUP:
            if (state.popup) {
                state = {...state, popup: null};
            }
            return state;
        case DONATION.MADE:
            // when donation is made, ensure that donation popup is hidden
            if (state.popup === UI.POPUP_DONATE) {
                state = {...state, popup: null};
            }
            return state;
    }

    return state;
}

function options(state={}, action) {
    if (action.type === OPTIONS.UPDATE && action.key in state) {
        let {key, value} = action;
        switch (key) {
            case 'maxViewportWidth':
                if (!isNaN(+value)) {
                    value = +value;
                }
                break;
            case 'autoHide':
                value = !!value
                break;
        }

        if (state[key] !== value) {
            state = {
                ...state,
                [key]: value
            };
        }
    }

    return state;
}

function donation(state={}, action) {
    switch (action.type) {
        case DONATION.SET_HANDLER:
            if (state.handler !== action.handler) {
                state = {
                    ...state,
                    handler: action.handler
                };
            }
            break;
        case DONATION.SET_PRODUCT:
            return {
                ...state,
                product: action.product
            };
        case DONATION.MADE:
            if (!state.made) {
                // looks a bit hacky: remove ads layer from page manually since
                // it was generated by script
                let elem = document.getElementById('carbonads');
                if (elem) {
                    elem.parentNode.removeChild(elem);
                }
                state = {
                    ...state,
                    made: true
                };
            }
            break;
        case DONATION.ENABLE_ADS_FALLBACK:
            if (!state.fallback) {
                state = {...state, fallback: true};
            }
            return state;
    }

    return state;
}

function breakpoints(state={}, action) {
    var selected = state.selected || [];
    let items = action.item || action.items || [];
    if (!Array.isArray(items)) {
        items = [items];
    }

    switch (action.type) {
        case BREAKPOINTS.SET_SELECTED:
            selected = items;
            break;

        case BREAKPOINTS.ADD_SELECTED:
            items = items.filter(id => selected.indexOf(id) === -1);
            if (items.length) {
                selected = selected.concat(items);
            }

            break;

        case BREAKPOINTS.REMOVE_SELECTED:
            let filtered = selected.filter(id => items.indexOf(id) === -1)
            if (filtered.length !== selected.length) {
                selected = filtered;
            }
            break;
    }

    if (selected !== state.selected) {
        state = {
            ...state,
            selected: selected
            .filter(id => state.items[id])
            .sort((a, b) => state.items[a].width - state.items[b].width)
        };
    }

    return state;
}

function removeItem(items, id) {
    return items.filter(item => item.id !== id);
}

/**
 * Ensures `deviceWallPicker.display` item always exists and points to valid
 * device or preset. If not, find most suitable
 * @param  {Object} state
 * @return {Object}
 */
function validateDeviceWallSelectedItem(state={}) {
    var curItem = state.deviceWallPicker && state.deviceWallPicker.display;
    var lookupKey = 'presets';

    if (curItem) {
        // make sure it points to existing item
        lookupKey = curItem.type === 'device' ? 'devices' : 'presets';
        let items = getItems(state, lookupKey);
        if (!items.some(item => item.id === curItem.id)) {
            curItem = null;
        }
    }

    // no valid item found, update state
    if (!curItem) {
        // pick first preset by default
        state = {
            ...state,
            deviceWallPicker: {
                ...state.deviceWallPicker,
                display: {
                    type: lookupKey === 'presets' ? 'preset' : 'device',
                    id: getItems(state, lookupKey)[0].id
                }
            }
        };
    }

    return state;
}

function mergeState(state={}, action) {
    if (action.type === APP.MERGE_STATE) {
        state = deepmerge(state, action.state || {});
    }

    return state;
}
