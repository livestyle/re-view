...
import {DEVICE_WALL, USER, UI, OPTIONS, APP, DONATION, BREAKPOINTS} from './action-names';
...
const validate = compose(
    forDonatorsOnly,
    validateDeviceWallSelectedItem
);

export default function(state, action) {
    state = keyReducers(state, action);
    state = mergeState(state, action);
...
function ui(state={}, action) {
    switch (action.type) {
        case UI.SET_MODE:
            ...
            return state;
        case UI.SET_POPUP:
            if (action.popup !== state.popup) {
                state = {...state, popup: action.poup};
            }
            return state;
        case UI.TOGGLE_POPUP:
            return {
                ...state,
                popup: state.popup === action.popup ? null : action.popup
            };
        case UI.RESET_POPUP:
            if (state.popup) {
...
function options(state={}, action) {
    if (action.type === OPTIONS.UPDATE && action.key in state) {
...
function donation(state={}, action) {
    switch (action.type) {
        case DONATION.SET_HANDLER:
            if (state.handler !== action.handler) {
                state = {
                    ...state,
...
function breakpoints(state={}, action) {
    ...
    let items = action.item || action.items || [];
    if (!Array.isArray(items)) {
        items = [items];
    }

    switch (action.type) {
        case BREAKPOINTS.SET_SELECTED:
            selected = items;
            break;

        case BREAKPOINTS.ADD_SELECTED:
            items = items.filter(id => selected.indexOf(id) === -1);
            if (items.length) {
        ...
        case BREAKPOINTS.REMOVE_SELECTED:
            let filtered = selected.filter(id => items.indexOf(id) === -1)
    ...
    if (selected !== state.selected) {
        state = {
            ...
            .sort((a, b) => state.items[a].width - state.items[b].width)
        };
    }
...
function forDonatorsOnly(state={}) {
    // always make sure `autoHide` option is enabled for donated users only
    // (this is for all those smarty-pants who knows how to use DevTools :)
    if (state.options.autoHide && !state.donation.made) {
        state = {
            ...state,
            options: {
                ...state.options,
...
function mergeState(state={}, action) {
    if (action.type === APP.MERGE_STATE) {
