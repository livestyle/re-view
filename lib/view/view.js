...
export default class View extends EventEmitter {
	...
	_updateLabel() {
		...
		if (this._downscale !== 1) {
			size += ` / ${this._downscale.toFixed(3).replace(/0+$/, '')}`;
		}
	...
	mount() {
		...
		if (!this._iframe.parentNode) {
			...
			.then(() => {
				this._iframeAvailable.resolve();
				this._pageSync = pageSync(this._iframe.contentWindow, {documentId: this.id});
				documentReady(this).then(meta => {
					this._iframeReady.resolve(meta);
					this.emit('document-ready', meta);
				})
				.catch(this._iframeReady.reject);
			})
	...
	unmount() {
		if (this._iframe) {
			...
			this._pageSync && this._pageSync.dispose();
		}
		return this._iframe;
	}

	sync(event, data) {
		this._pageSync && this._pageSync(event, data);
	}
