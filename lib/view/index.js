    ...
    return new FSM({
        ...
        show: {
            _enter() {
                ...
                console.log('show view for state', curState);
...
function getItems(state, key) {
    var lookup = {};
    return [].concat(getKey(state, `user.${key}`), getKey(state, key))
    // keep unique items only
    .filter(item => !item || item.id in lookup ? false : lookup[item.id] = true);
}
