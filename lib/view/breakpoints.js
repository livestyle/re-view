...
export default function(container, url, specs, options={}) {
	...
	var views = specs.map(spec => new View(url, {
		...spec,
	...
	return {
		...
		updateOptions(options={}) {
			if ('maxViewportWidth' in options) {
				options = {maxViewportWidth: getMaxViewportWidth(options)}
				views.forEach(view => view.updateOptions(options));
			}
		},
		destroy() {
			sync && sync.destroy();
